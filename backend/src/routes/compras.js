...
router.post('/', async (req, res) => {
    await ComprasService.create(req.body.data, req.currentUser, true, req.headers.referer);
    const payload = true;
...
router.get('/', wrapAsync(async (req, res) => {
  ...
  const payload = await ComprasDBApi.findAll(
    req.query,
  );
  if (filetype && filetype === 'csv') {
    const fields = ['id','razonSocial','numeroComprobante',

        'mongoGravado10','mongoGravado5','exento',
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await ComprasDBApi.findBy(
    { id: req.params.id },
  );
