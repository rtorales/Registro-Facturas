...
import * as dataFormat from 'pages/CRUD/Ventas/table/VentasDataFormatters';
...
const VentasTable = () => {
  ...
  const count = useSelector((store) => store.ventas.list.count);
  const modalOpen = useSelector((store) => store.ventas.list.modalOpen);
  const rows = useSelector((store) => store.ventas.list.rows);
  const idToDelete = useSelector((store) => store.ventas.list.idToDelete);
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Ventas'
                : 'http://localhost:8080/api-docs/#/Ventas'
            }
          >
            API documentation for ventas
          </LinkMaterial>
        </div>
      </Widget>
