...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/ventas/autocomplete`, { params });
  return response.data;
}

const VentasSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.ventas.hasPermissionToCreate,
});

export default connect(select)(VentasSelectItem);
