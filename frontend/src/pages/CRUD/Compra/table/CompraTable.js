...
import * as dataFormat from 'pages/CRUD/Compra/table/CompraDataFormatters';

// eslint-disable-next-line
import * as contribuyenteDataFormat from 'pages/CRUD/Contribuyente/table/ContribuyenteDataFormatters';

import actions from 'actions/compra/compraListActions';
...
const CompraTable = () => {
  ...
  const count = useSelector((store) => store.compra.list.count);
  const modalOpen = useSelector((store) => store.compra.list.modalOpen);
  const rows = useSelector((store) => store.compra.list.rows);
  const idToDelete = useSelector((store) => store.compra.list.idToDelete);
  ...
  const getCompraCSV = async () => {
    const response = await axios({url: '/compra?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'compraCSV.csv'
    link.click()
  };
  ...
  const columns = [
      ...
      { field: "contribuyente",
        ...
        sortable: false,
        renderCell: (params) => contribuyenteDataFormat.listFormatter(params.row[params.field], history, 'contribuyente'),
        flex: 1,
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="compra" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Compra')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/compra/new" className={classes.element}>
          ...
          <Button type='button' variant='contained' onClick={getCompraCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/compra/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Compra'
                : 'http://localhost:8080/api-docs/#/Compra'
            }
          >
            API documentation for compra
          </LinkMaterial>
        </div>
      </Widget>
