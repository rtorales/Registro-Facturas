...
import * as dataFormat from 'pages/CRUD/Venta/table/VentaDataFormatters';

// eslint-disable-next-line
import * as contribuyenteDataFormat from 'pages/CRUD/Contribuyente/table/ContribuyenteDataFormatters';

import actions from 'actions/venta/ventaListActions';
...
const VentaTable = () => {
  ...
  const count = useSelector((store) => store.venta.list.count);
  const modalOpen = useSelector((store) => store.venta.list.modalOpen);
  const rows = useSelector((store) => store.venta.list.rows);
  const idToDelete = useSelector((store) => store.venta.list.idToDelete);
  ...
  const getVentaCSV = async () => {
    const response = await axios({url: '/venta?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'ventaCSV.csv'
    link.click()
  };
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="venta" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Venta')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/venta/new" className={classes.element}>
          ...
          <Button type='button' variant='contained' onClick={getVentaCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/venta/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Venta'
                : 'http://localhost:8080/api-docs/#/Venta'
            }
          >
            API documentation for venta
          </LinkMaterial>
        </div>
      </Widget>
