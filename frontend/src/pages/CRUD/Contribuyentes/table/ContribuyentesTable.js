...
const ContribuyentesTable = () => {
  ...
  const idToDelete = useSelector((store) => store.contribuyentes.list.idToDelete);
  ...
  const getContribuyentesCSV = async () => {
    const response = await axios({url: '/contribuyentes?filetype=csv', method: 'GET',responseType: 'blob'});
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="contribuyentes" openModal={openModal} {...params} />,
      }
  ];
    ...
    <div>
      <Widget title={<h4>{humanize('Contribuyentes')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          ...
          <Button type='button' variant='contained' onClick={getContribuyentesCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/contribuyentes/${e.id}/edit`)}}
            autoHeight
          />
        </div>
