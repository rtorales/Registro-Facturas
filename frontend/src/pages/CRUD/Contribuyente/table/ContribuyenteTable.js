...
import * as dataFormat from 'pages/CRUD/Contribuyente/table/ContribuyenteDataFormatters';

import actions from 'actions/contribuyente/contribuyenteListActions';
...
const ContribuyenteTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    {label: 'Razon Social', title: 'razonSocial'},{label: 'Ruc', title: 'ruc'},
  ...
  const count = useSelector((store) => store.contribuyente.list.count);
  const modalOpen = useSelector((store) => store.contribuyente.list.modalOpen);
  const rows = useSelector((store) => store.contribuyente.list.rows);
  const idToDelete = useSelector((store) => store.contribuyente.list.idToDelete);
  ...
  const getContribuyenteCSV = async () => {
    const response = await axios({url: '/contribuyente?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'contribuyenteCSV.csv'
    link.click()
  };
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="contribuyente" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Contribuyente')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/contribuyente/new" className={classes.element}>
          ...
          <Button type='button' variant='contained' onClick={getContribuyenteCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/contribuyente/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Contribuyente'
                : 'http://localhost:8080/api-docs/#/Contribuyente'
            }
          >
            API documentation for contribuyente
          </LinkMaterial>
        </div>
      </Widget>
